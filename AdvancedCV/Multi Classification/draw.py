...
def plot_dual_axis_pic(ax1_data: Iterable, ax2_data: Iterable,
                       save_path: os.PathLike, x_label: str, y1_label: str,
                       y2_label: str) -> None:
    '''
    ...
    for label, X, Y in ax1_data:
        ax1.plot(X, Y, label=label,marker=".")

    ax2 = ax1.twinx()
    ax2.set_ylabel(y2_label)
    for label, X, Y in ax2_data:
        ax2.plot(X, Y, label=label,marker="^")
...
def plot_single_axis_pic(data: Iterable, save_path: os.PathLike, x_label: str,
                         y_label: str) -> None:
    '''
    This function make a dual axes plot and save it to specified path
...
def draw_statistics(statistics):
    ...
    plot_dual_axis_pic(
        ax1_data=[[
            "train loss", statistics['train_x'], statistics['train_loss']
        ], ["val loss", statistics['val_x'], statistics['val_loss']]],
        ax2_data=[
            [
                "train accurancy", statistics['train_x'],
                statistics['train_acc']
            ],
            ["val accurancy", statistics['val_x'], statistics['val_acc']],
    ...
    plot_single_axis_pic(data=[[
        f"train acc. of category{i+1}", statistics['train_x'],
        statistics['train_acc_full'][:, i].tolist()
    ] for i in range(6)],
                         x_label="epoch",
                         y_label="train_accurancy",
                         save_path=os.path.join(pics_path, "train_acc.png"))
    plot_single_axis_pic(data=[[
        f"val. acc. of category{i+1}", statistics['val_x'],
        statistics['val_acc_full'][:, i].tolist()
    ] for i in range(6)],
                         x_label="epoch",
                         y_label="val_accurancy",
                         save_path=os.path.join(pics_path, "val_acc.png"))
